...
def latest_feed_diverse(limit=5):
    """Source-diverse home teaser: the newest item from EACH source, ordered by
    which source posted most recently. A high-volume source (Aernbot Notebook posts
    ~3x/day) no longer crowds out a once-daily digest, so every producer surfaces."""
    by_source = feed_sources(per_source=1)  # already ordered by each source's recency
    newest = [items[0] for items in by_source.values() if items]
    return newest[:limit]
